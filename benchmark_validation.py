...
def run_benchmarking(clustering_alg:str, radius:float, base_path:str)->None:
    """
    Run benchmarking.
    Args:
        clustering_alg (str): Clustering algorithm, for plot names.
        radius (float): HCM cutoff distance.
        base_path (str): String with pathway containing datasets csvs, also output location.
    Returns:
        None.
    """
    ...
    # cluster dataset
    clustered_data_path = base_path + f"{clustering_alg}_asteroid_clusters_full_r{r_str}.csv"
    ...
    plt.savefig(f"{clustering_alg}_r{r_str}_purity.png", dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
    ...
    plt.savefig(f"{clustering_alg}_r{r_str}_completeness.png", dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
    ...
    output_table.to_csv(base_path + clustering_alg + f"cluster_validation_correctness_r{r_str}.csv", index=False)
    ...
    # export #
    family_validation.to_csv(base_path + clustering_alg + f"family_validation_summary_r{r_str}.csv", index=False)
